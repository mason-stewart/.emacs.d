...
#+TITLE: Emacs 24 configuration for Mason Stewart
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; make scrolling not insanely slow
...
Some helpful settings for =tabbar-mode=.
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq mws/group-by-project nil)
  ;; what's going on with tabbar and flycheck?
  (defun mws/tabbar-buffer-groups ()
      ...
      (t
       ...
       (let ((group
         ...
         (if (projectile-project-p)
             (if mws/group-by-project
                 (projectile-project-name)
               (format "%s:%s" (projectile-project-root) group))
           group))
       ))))

  (defun mws/cached (func)
  ...
  (setq cached-ppn (mws/cached 'mws/tabbar-buffer-groups))

  (defun mws/tabbar-groups-by-project ()
    (funcall cached-ppn (buffer-name)))

  (setq tabbar-buffer-groups-function 'mws/tabbar-groups-by-project)

  (defun mws/toggle-group-by-project ()
    (interactive)
    (setq mws/group-by-project (not mws/group-by-project))
    (message "Grouping by project alone: %s"
             (if mws/group-by-project "enabled" "disabled"))
    (setq cached-ppn (mws/cached 'mws/tabbar-buffer-groups)))


  (setq tabbar-buffer-groups-function 'mws/tabbar-buffer-groups)
#+END_SRC
...
** Javascript
   ...
   N.B., ac-js2 *isn't* installed via MELPA, as the current version had
...
** Purescript
   =inferior-psci-mode= offers a few helpful key-bindings for loading
   current module/file into the repl.

   I've also added =psci-multi-line=, which is janky, but if you /need/
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun psci-multi-line ()
    (interactive)
    (let ((previous-args psci/arguments))
      ;; temporarily set multi-line-mode on with -m.
      ;; the nil value is the STARTFILE which we don't need
      ;; C-h f "make-comint-in-buffer" for more details.
      (setq psci/arguments '(nil "-m"))
      ;; fire up psci in the context of the current buffer
      (psci)
      ;; and set the args back to their original value
...
#+BEGIN_SRC emacs-lisp
  (add-hook 'ruby-mode-hook
            (lambda ()
              ...
              ;; (ac-robe-setup) turned off for now
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "A-]") 'tabbar-forward)
  (global-set-key (kbd "A-[") 'tabbar-backward)
  (global-set-key (kbd "A-M-]") 'tabbar-forward-group)
  (global-set-key (kbd "A-M-[") 'tabbar-backward-group)
  ...
  ;; org-mode rebuild project
  (global-set-key (kbd "A-r") 'org-publish-current-project)

  ;; use Esc as C-g for when muscle memory kicks in
  (global-set-key (kbd "<escape>") 'keyboard-quit)

  ;; jump 5 lines
  (global-set-key (kbd "A-C-n") (lambda () (interactive) (next-line 5)))
  (global-set-key (kbd "A-C-p") (lambda () (interactive) (previous-line 5)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (setq org-publish-project-alist
        '(("orgfiles"
           :base-directory "~/masondesu.com/"
           :base-extension "org"
           :publishing-directory "~/masondesu.com/dist/"
           :publishing-function org-html-publish-to-html
           :exclude "TO_BE_EXCLUDED_OMG.org"   ;; regexp
           :headline-levels 3
           :section-numbers nil
           :with-toc t
           :html-html5-fancy t
           ...
           :html-preamble "<div class='color'></div>
                                 <div class='fullscreen'></div>"
           :html-head-extra ""
           :html-postamble "<p class='postamble'>Written by %a. Last Updated %T. Made with %c</p>
                                  <script type='text/javascript' src='/js/cool.js'></script>")
          
          ("images"
           :base-directory "~/masondesu.com/images/"
           :base-extension "jpg\\|gif\\|png"
           :publishing-directory "~/masondesu.com/dist/images/"
           :publishing-function org-publish-attachment)

          ("fonts"
           :base-directory "~/masondesu.com/fonts/"
           :base-extension "eot\\|svg\\|ttf\\|woff"
           :publishing-directory "~/masondesu.com/dist/fonts/"
           :publishing-function org-publish-attachment)
          
          ("css"
           :base-directory "~/masondesu.com/css/"
           :base-extension "css"
           :publishing-directory "~/masondesu.com/dist/css/"
           :publishing-function org-publish-attachment)

          ("js"
           :base-directory "~/masondesu.com/js/"
           :base-extension "js"
           :publishing-directory "~/masondesu.com/dist/js/"
           :publishing-function org-publish-attachment)
          
          ("website" :components ("orgfiles" "images" "fonts" "css" "js"))))
#+END_SRC
